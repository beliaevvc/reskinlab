...
export function ProjectPage() {
  ...
  const handleCompleteProject = () => {
    setConfirmAction({
      type: 'complete',
      title: t('actions.completeProject'),
      message: t('stages.confirm', { name: project?.name }),
    });
  };
  ...
  const handleArchiveProject = () => {
    setConfirmAction({
      type: 'archive',
      title: t('actions.archiveProject'),
      message: t('stages.warning', { name: project?.name }),
    });
  };
    ...
    return (
      <div className="flex items-center justify-center h-[calc(100vh-4rem)]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-neutral-400">{t('detail.loading')}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="p-6">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6">
          <p className="text-sm text-red-700">{t('detail.loadError')}: {projectError.message}</p>
        </div>
      </div>
    );
  }
          ...
          <div className="px-4 lg:px-5 py-2 flex items-center justify-between flex-shrink-0 border-b border-neutral-100 bg-white/60 backdrop-blur-sm">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] text-neutral-400 tabular-nums">
                {t('tasks.taskCount', { count: taskStats.total })}
              </span>
            </div>
            {effectiveIsStaff && (
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-lg shadow-xl max-w-sm w-full p-6">
            ...
            <div className="flex gap-3">
              ...
              >
                {confirmAction.type === 'archive' ? t('actions.archive') : t('actions.complete')}
              </button>
            </div>
          </div>
        </div>
  ...
  if (columnTasks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-neutral-300">
        ...
        <span className="text-xs">{t('tasks.noTasksInColumn')}</span>
      </div>
    );
  }
