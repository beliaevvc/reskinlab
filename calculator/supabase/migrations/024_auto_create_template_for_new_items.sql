-- ===========================================
-- AUTO CREATE TEMPLATE FOR NEW ITEMS IN SPECIFICATIONS
-- ===========================================

-- Функция для автоматического создания шаблона при первом использовании нового item_id
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    -- Получаем название из функции get_item_task_name
    v_task_title := get_item_task_name(p_item_id);
    
    -- Создаем новый шаблон с дефолтными значениями
...
-- Обновляем функцию создания задач, чтобы она автоматически создавала шаблон для новых item_id
  ...
  IF NEW.status = 'paid' AND (OLD.status IS NULL OR OLD.status != 'paid') THEN
    ...
    IF v_settings.spec_tasks_enabled AND v_spec_state->'items' IS NOT NULL THEN
      ...
      FOR v_item_id, v_item_data IN SELECT * FROM jsonb_each(v_spec_state->'items') LOOP
        ...
        -- ОБЕСПЕЧИВАЕМ СУЩЕСТВОВАНИЕ ШАБЛОНА ДЛЯ ЭТОГО item_id
        PERFORM ensure_spec_item_template_exists(v_item_id);
        
        -- Получаем шаблон для этого item_id (теперь он точно существует)
...
-- Комментарий
COMMENT ON FUNCTION ensure_spec_item_template_exists(TEXT) IS 'Автоматически создает шаблон для item_id, если его еще нет. Используется при создании задач из спецификации для новых пунктов калькулятора.';
