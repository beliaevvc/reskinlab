-- ===========================================
-- PROJECT STATUS WORKFLOW UPDATES
-- ===========================================

-- 1. Обновляем CHECK constraint для статусов проектов
-- Добавляем статусы: in_production, archived
...
CHECK (status IN (
  'draft',           -- Черновик (до акцепта оффера)
  'pending_payment', -- Ожидает оплаты
  'active',          -- Активен (оффер акцептирован, но оплата еще не подтверждена)
  'in_production',   -- В продакшене (первая оплата подтверждена, задачи созданы)
  'on_hold',         -- Приостановлен
  'completed',       -- Завершен (все файлы приняты, админ/AM завершил проект)
  'cancelled',       -- Отменен
  'archived'         -- Архивирован
));

-- 2. Функция для обновления статуса проекта при акцепте оффера
CREATE OR REPLACE FUNCTION update_project_status_on_offer_accepted()
RETURNS TRIGGER AS $$
DECLARE
  v_project_id UUID;
BEGIN
  -- Проверяем, что статус оффера изменился на 'accepted'
  IF NEW.status = 'accepted' AND (OLD.status IS NULL OR OLD.status != 'accepted') THEN
    -- Получаем project_id через specification_id
    ...
    IF v_project_id IS NOT NULL THEN
      ...
      RAISE NOTICE 'Project % status updated to active (offer accepted)', v_project_id;
    END IF;
  END IF;
...
-- 3. Создаем триггер для обновления статуса проекта при акцепте оффера
DROP TRIGGER IF EXISTS trigger_update_project_status_on_offer_accepted ON public.offers;
CREATE TRIGGER trigger_update_project_status_on_offer_accepted
  AFTER UPDATE OF status ON public.offers
  FOR EACH ROW
  WHEN (NEW.status = 'accepted' AND OLD.status != 'accepted')
  EXECUTE FUNCTION update_project_status_on_offer_accepted();

-- 4. Обновляем функцию auto_create_tasks_on_first_payment() для изменения статуса на in_production
...
-- 5. Функция для завершения проекта (completed)
CREATE OR REPLACE FUNCTION complete_project(project_uuid UUID)
RETURNS void AS $$
BEGIN
  -- Проверяем права доступа (только админ или AM)
  IF NOT (is_admin() OR is_am()) THEN
    RAISE EXCEPTION 'Access denied. Only admins and AMs can complete projects';
  END IF;
  ...
  WHERE id = project_uuid
    AND status IN ('in_production', 'active', 'on_hold');
  
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Project % not found or cannot be completed', project_uuid;
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 6. Функция для архивирования проекта
CREATE OR REPLACE FUNCTION archive_project(project_uuid UUID)
RETURNS void AS $$
BEGIN
  -- Проверяем права доступа (только админ или AM)
  IF NOT (is_admin() OR is_am()) THEN
    RAISE EXCEPTION 'Access denied. Only admins and AMs can archive projects';
  END IF;
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'Project % not found or cannot be archived (only completed or cancelled projects can be archived)', project_uuid;
  END IF;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 7. RLS политики для функций завершения и архивирования (только админы и AM)
-- Эти функции используют SECURITY DEFINER, поэтому проверяем права через is_admin() или is_am()

-- Комментарии
COMMENT ON FUNCTION update_project_status_on_offer_accepted() IS 'Обновляет статус проекта на active при акцепте оффера';
COMMENT ON FUNCTION complete_project(UUID) IS 'Завершает проект (меняет статус на completed). Доступно админу и AM';
COMMENT ON FUNCTION archive_project(UUID) IS 'Архивирует проект (меняет статус на archived). Доступно админу и AM. Только для завершенных или отмененных проектов';
